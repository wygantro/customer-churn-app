...
server <- function(input, output) {
  ...
  observe({
    ...
    react$cat_percent <- react$churn_modeling %>%
      ...
      summarise(count_percent = sum(Exited == 'yes')/(n())*100) %>%
  ...
  output$plot1 <- renderPlot({
    ...
    plot1 <- ggplot(react$churn_modeling_perc, aes(x = Exited, y = count_percent, fill=Exited)) +
      ...
      annotate("text", x=2, y=as.numeric(react$churn_modeling_perc[3,2])*1.1, label="baseline", size=4, color="black") +
      theme_bw()
    plot1
  })
  ...
  #pareto of percent churned by factors within selected categories
  output$plot2 <- renderPlot({
    plot2 <-  ggplot(react$cat_percent, mapping = aes(x=count_percent, y=reorder(.data[[input$SelectCategory_ID_1]], count_percent))) + 
      ...
      labs(y= "Category Factors", x = "Percent Churned (%)") +
